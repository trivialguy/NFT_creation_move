...
module overmind::NonFungibleToken {
    ...
    */
    fun init(ctx: &mut TxContext) {
        transfer::transfer(MinterCap{id: object::new(ctx), sales: balance::zero()}, tx_context::sender(ctx))
    }
    ...
    */
    public fun mint_nft(
        ...
        event::emit(NonFungibleTokenMinted{nft_id: object::uid_to_inner(&new_nft.id),recipient: recipient});
    ...
    */
    public fun combine_nfts(
        ...
        let desc: String = string::utf8(b"Combined NFT of ");
        let plus: String= string::utf8(b" + ");
        let and: String = string::utf8(b" and ");
        string::append(&mut desc,nft1.name);
        string::append(&mut desc,and);
        string::append(&mut desc,nft2.name);
        string::append(&mut nft1.name,plus);
        string::append(&mut nft1.name,nft2.name);
        let new_nft=NonFungibleToken{id: object::new(ctx),
        name: nft1.name,
        description: desc,
        image: url::new_unsafe_from_bytes(new_image_url)
        };
        event::emit(NonFungibleTokenCombined{nft1_id: object::uid_to_inner(&nft1.id),nft2_id: object::uid_to_inner(&nft2.id),new_nft_id: object::uid_to_inner(&new_nft.id)});
    ...
    */
    public fun withdraw_sales(
        ...
        // transfer::public_transfer(sales,tx_context::sender(ctx));
        sales
    }
